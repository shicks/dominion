...
module Network.Websocket( Config(..), ConfigRestriction(..), WS(..),
    ...
    restrictionValid :: String -> ConfigRestriction -> Bool
    restrictionValid _ Any = True
    ...
    parseRequest req = do
      upgrade  <- lookupField (FkOther $ pack "Upgrade") req
      origin   <- lookupField (FkOther $ pack "Origin") req
      host     <- lookupField FkHost req
      hostURI  <- parseURI (pack $ "ws://" ++ unpack host ++ "/")
    ...
    accept config socket =
        forever $ doWebSocket socket $ \h req ->
            do let (upgrade, origin, hostDomain) = case parseRequest req of
               ...
               return $ assert (unpack upgrade == "WebSocket") ()
               return $ assert (restrictionValid (unpack origin) (configOrigins config)) ()
               return $ assert (restrictionValid (unpack hostDomain) (configDomains config)) ()

               sendHandshake h (unpack origin) location
